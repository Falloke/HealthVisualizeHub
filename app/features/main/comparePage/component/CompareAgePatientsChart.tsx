...
// tooltip แสดงสองจังหวัด
function AgeCompareTooltip({
  active,
  payload,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const url = `/api/compareInfo/age-group?start_date=${start_date}&end_date=${end_date}&mainProvince=${encodeURIComponent(
          mainProvince!
        ...
        if (!res.ok) {
          throw new Error(
            text || "โหลดข้อมูลเปรียบเทียบผู้ป่วยตามช่วงอายุไม่สำเร็จ"
          );
        }
    ...
    () =>
      niceMax(
        Math.max(
          0,
          ...rows.map((d) =>
            Math.max(Number(d.mainPatients ?? 0), Number(d.comparePatients ?? 0))
          )
        )
      ),
    [rows]
  ...
  return (
    <div className="rounded bg-white p-4 shadow">
      <h4 className="mb-2 font-bold">
        เปรียบเทียบผู้ป่วยสะสมรายช่วงอายุ{" "}
        {hasBoth
          ? `(${mainProvince} vs ${compareProvince})`
          : "(เลือกจังหวัดหลักและจังหวัดเปรียบเทียบให้ครบ)"}
      </h4>
