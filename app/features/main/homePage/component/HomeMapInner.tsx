...
export default function HomeMapInner({ className = "" }: { className?: string }) {
  ...
  // ✅ Cache count per province/date/disease
  const countsCache = useRef<Map<string, number>>(new Map());
  useEffect(() => {
    countsCache.current.clear();
  }, [start_date, end_date, diseaseCode]);

  // ✅ กัน race: hover ใหม่แล้ว response เก่ามาทับ
  const hoverSeqRef = useRef(0);

  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`/data/thailand-province-simple.json?v=${Date.now()}`, {
          cache: "no-store",
        });
  ...
  useEffect(() => {
    (async () => {
      try {
        setProvErr(null);
        const res = await fetch(`/data/Thailand-ProvinceName.json?v=${Date.now()}`, {
          cache: "no-store",
        });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  const getThaiOfficialName = useCallback(
  ...
  /**
   * ✅ ดึงจำนวนผู้ป่วยของจังหวัด (ส่ง disease ไปด้วย)
   * ✅ รองรับ response หลายรูปแบบ totalPatients/patients/count
   * ✅ กัน race condition (hover ไว)
   */
  const fetchPatientsForProvince = useCallback(
    async (provinceName: string): Promise<number> => {
      ...
      if (!res.ok) {
        console.error("❌ patients-summary failed:", { status: res.status, url, text });
      ...
      const n = Number(json?.totalPatients ?? json?.patients ?? json?.count ?? 0);
      const safe = Number.isFinite(n) ? n : 0;

      countsCache.current.set(key, safe);
      return safe;
    },
      ...
      onEachFeature: (feature: Feature, layer: L.Layer) => {
        ...
        layer.on({
          mouseover: async (e: L.LeafletMouseEvent) => {
            ...
            // ✅ กล่องข้อมูลขึ้นก่อนเลย แล้วค่อยโหลดเลข
            ...
            // ✅ กัน hover ไว (race)
            const mySeq = ++hoverSeqRef.current;

            try {
              const p = await fetchPatientsForProvince(nameTH);

              // ถ้ามี hover ใหม่เข้ามาแล้ว ไม่ต้องอัพเดตตัวเก่า
              if (mySeq !== hoverSeqRef.current) return;
          ...
          mouseout: (e: L.LeafletMouseEvent) => {
            ...
            // ✅ เปลี่ยน seq เพื่อกัน response เก่ามาทับ
            hoverSeqRef.current++;
    ...
    <div ref={wrapperRef} className={`relative z-0 w-full ${className}`}>
      ...
      <div className="pointer-events-none absolute bottom-3 left-3 z-[1000] space-y-1 rounded-md border bg-white/90 p-2 text-xs shadow">
        ...
        <div className="flex items-center gap-2">
          <span className={`h-3 w-3 rounded ${RISK_META["สูงมาก"].bg}`} />
          <span>สูงมาก</span>
        </div>
        <div className="flex items-center gap-2">
          <span className={`h-3 w-3 rounded ${RISK_META["สูง"].bg}`} />
          <span>สูง</span>
        </div>
        <div className="flex items-center gap-2">
          <span className={`h-3 w-3 rounded ${RISK_META["ปานกลาง"].bg}`} />
          <span>ปานกลาง</span>
        </div>
        <div className="flex items-center gap-2">
          <span className={`h-3 w-3 rounded ${RISK_META["ต่ำ"].bg}`} />
          <span>ต่ำ</span>
        </div>
      </div>
    </div>
