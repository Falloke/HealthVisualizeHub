...
async function queryProvinceDeaths(opts: {
  ...
  if (!fact) return { province: opts.provinceNameTh, deaths: 0 };

  const resolved = await resolveDiseaseCode(opts.disease);
  if (!resolved) return { province: opts.provinceNameTh, deaths: 0 };

  const diseaseIn = diseaseCandidates(resolved);
  if (diseaseIn.length === 0) return { province: opts.provinceNameTh, deaths: 0 };
...
export async function GET(req: NextRequest) {
  try {
    ...
    const [main, compare] = await Promise.all([
      mainProvince
        ? queryProvinceDeaths({ start_date, end_date, provinceNameTh: mainProvince, disease })
        : Promise.resolve(undefined),
      compareProvince
        ? queryProvinceDeaths({ start_date, end_date, provinceNameTh: compareProvince, disease })
        : Promise.resolve(undefined),
    ]);
