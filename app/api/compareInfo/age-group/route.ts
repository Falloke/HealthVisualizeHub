...
/** ✅ Resolve จังหวัดจาก ref.provinces_moph (รับได้ทั้งเลขและชื่อไทย) */
...
async function queryAgePatients(args: {
  ...
  if (!provinceId) return (AGE_ORDER as unknown as string[]).map((k) => ({ ageRange: k, patients: 0 }));
...
export async function GET(req: NextRequest) {
  try {
    ...
    const [mainRows, compareRows] = await Promise.all([
      queryAgePatients({ diseaseId, start_date, end_date, provinceNameTh: mainProvince }),
      queryAgePatients({ diseaseId, start_date, end_date, provinceNameTh: compareProvince }),
    ]);
